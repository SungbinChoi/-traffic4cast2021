...
other_city_norm_list = [0.03, 0.06, 0.08, 0.47, 0.31, 0.06, 0.38, 0.04]
other_city_norm_list = np.array(other_city_norm_list, np.float32)
other_city_norm_list /= np.min(other_city_norm_list)               
other_city_norm_list = np.power(1.0/other_city_norm_list, 0.5)
...
if __name__ == '__main__':
  ...
  loss_func3 = nn.MSELoss(reduction='none')  
  ...
  def load_train_multithread():
    while True:
      ...
      train_city_norm_list = []
      for train_i_j in i_j_list:
          (i,j) = train_i_j
          file_path = train_data_filepath_list[i]
          city_norm = other_city_norm_list[train_static_data_index_list[i]]
          train_city_norm_list.append(city_norm)
      ...
      train_city_norm_list = np.array(train_city_norm_list, np.float32)
      ...
      train_output_queue.put( (input_data, true_label, train_city_norm_list.reshape(-1,1,1,1).copy()) )
  thread_list = []
  assert num_thread > 0
  ...
  for epoch in range(num_epoch_to_train):
    ...
    for a in range(num_iteration_per_epoch):
      ...
      while train_output_queue.empty():
        time.sleep(0.1)
      (input_data, true_label, train_city_norm_list) = train_output_queue.get()
      optimizer.zero_grad()
      input  = torch.from_numpy(input_data).float().cuda() 
      target = torch.from_numpy(true_label).float().cuda() 
      city_loss_weight  = torch.from_numpy(train_city_norm_list).float().cuda() 
      prediction = net(input)
      loss = torch.mean(loss_func3(prediction, target) * city_loss_weight)
      sum_train_iter += 1
