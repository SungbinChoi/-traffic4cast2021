...
target_city   = 'ISTANBUL'        # 'BERLIN' 'CHICAGO' 'ISTANBUL' 'MELBOURNE'             'NEWYORK'  'VIENNA'                          
load_model_id = 't1m7'                                                                 
load_model_path  = '../trained_models/' + load_model_id + '.pth'
...
class Conv5x5Block(nn.Sequential):
    def __init__(self, 
                 in_size: int, 
                 h_size: int, ) -> None:
        super(Conv5x5Block, self).__init__()
        self.add_module('conv', nn.Conv2d(in_size, h_size, kernel_size=5, stride=1, padding=2, bias=True))
        self.add_module('elu',  nn.ELU(inplace=True))                                        
        self.add_module('norm', nn.GroupNorm(num_groups=num_groups, num_channels=h_size))    
...
class DownBlock(nn.Module):
    def __init__(self, 
        ...
        self.conv_1 = Conv1x1Block( in_size=in_size,  h_size=h_size)
        
        self.conv_2 = Conv1x1Block( in_size=h_size,   h_size=h_size)
        self.conv_3 = Conv3x3Block( in_size=h_size,   h_size=h_size)
        
        self.conv_4 = Conv1x1Block( in_size=h_size,   h_size=h_size)
        self.conv_5 = Conv3x3Block( in_size=h_size,   h_size=h_size)
        
        self.pool_1 = MaxBlock(kernel_size=3, stride=1, padding=1)
        self.conv_6 = Conv3x3Block( in_size=h_size,   h_size=h_size)
        
        self.pool_2 = MaxBlock(kernel_size=5, stride=1, padding=2)
        self.conv_7 = Conv3x3Block( in_size=h_size,   h_size=h_size)
        
        self.conv_8 = Conv1x1Block( in_size=h_size,   h_size=h_size)
        self.conv_9 = Conv3x3Block( in_size=h_size,   h_size=h_size)
        
        self.conv_out = Conv3x3Block( in_size=h_size,   h_size=out_size)
...
class NetA(nn.Module):
    ...
    def __init__(self,):
        ...
        self.block0 = DownBlock(in_size=NUM_INPUT_CHANNEL, h_size=64, out_size=128, do_pool=False)
        self.block1 = DownBlock(in_size=128, h_size=64, out_size=128,)
        self.block2 = DownBlock(in_size=128, h_size=64, out_size=128, )
        self.block3 = DownBlock(in_size=128, h_size=64, out_size=128, )
        self.block4 = DownBlock(in_size=128, h_size=64, out_size=128, )
        self.block5 = DownBlock(in_size=128, h_size=64, out_size=128, )
        self.block6 = DownBlock(in_size=128, h_size=64, out_size=128,)
        self.block7 = DownBlock(in_size=128, h_size=64, out_size=128,)
